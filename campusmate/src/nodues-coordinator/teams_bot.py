"""Teams Activity Protocol passthrough handler for nodues-coordinator.
...
looks the officer up in the shared identity store, (re)creates the
...
logger = logging.getLogger("nodues-coordinator.teams")

# Friendly informative-update text per tool, for the Teams "blue bar".
_TOOL_STATUS = {
    "check_dues": "Checking dues status…",
    "clear_due": "Clearing department due…",
    "request_approval": "Requesting officer approval…",
    "issue_no_dues_certificate": "Issuing no-dues certificate…",
    "save_request": "Saving no-dues dossier…",
}
...
async def _run_agent_streamed(text: str, turn: StreamingTurn) -> str:
    ...
    await turn.informative("CampusMate is on it…")
        ...
        async for update in stream:
            for content in getattr(update, "contents", []) or []:
                ...
                if ctype == "function_call":
                    ...
                    if res is not None and name in ("check_dues", "save_request"):
    ...
    if not accumulated.strip():
        accumulated = "(CampusMate completed without a text response.)"
...
async def _handle_approval_decision(
    ...
    card = teams_cards.build_decision_result_card(
        action=value.get("action") or "No-dues certificate approval",
...
async def send_approval_card(
    *, action: str, description: str, severity: str, approval_id: str
    ...
    Looks the officer up in the shared identity store, (re)creates the conversation via
    the Bot Connector if needed, then sends the adaptive card. Returns success.
    """
    ref = store.get_approver()
    if ref is None:
        logger.warning(
            "send_approval_card: no stored conversation ref — the officer must DM the agent "
